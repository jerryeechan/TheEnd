...
public class TileMaster : EditorWindow {
	...
	static void ResetManager()
	{
		...
		if(cmquad == null)
		{
			...
			Renderer renderer = cmquad.GetComponent<Renderer>();
			renderer.enabled = false; //disable quad's renderer
			
			EditorUtility.SetSelectedWireframeHidden(renderer, true); //hide wireframe from wireframe view mode
		}
		if(window != null)
		{
